  ...
  bool init(hardware_interface::EffortJointInterface *hw, ros::NodeHandle &n)
    ...
    {	   
      ...
      //  id_solver_->JntToCoriolis(q_, qdot_, C_);
        /// doesn't this already do the whole gravity compensation though??although!!!
        // I see it initialised but not acutal implemented so probably not? i guess pff fml
      ...
      //  }
        ...
        // ********** 2. Gravity Compensation and PD control... ********//
        //Basically doing what has already been done on line 179... thus just using that id_solver_
	//KDL::Vector gravity(0.0,0.0,-9.81); 
	//KDL::ChainDynParam dyn_param(kdl_chain_, gravity); // initialises object to allow for dynamic calculation
	//KDL::JntArray gravity_torques(n_joints_); // calculate gravitational torques on each joint
	
	 id_solver_->JntToGravity(q_, G_);  //calculates gravitational torques on each joint
         // PD stuff  
         // 1st error between desired and actual joint (also above also slide 8)
         e_.data = qd_.data - q_.data;
	 e_dot_.data = qd_dot_.data - qdot_.data;
         
                 // 2nd is calculate the new torues + add on the gravity compensation as seen in slide 25
	// now basically setting position compesating for gravity
         tau_d_.data = G_.data + Kp_.data.cwiseProduct(e_.data) - Kd_.data.cwiseProduct(qdot_.data); 
         //with feedback on q dot (qdot needs to be externally generated and then plugged in derivative gain)
         tau_d_.data = G_.data + Kp_.data.cwiseProduct(e_.data) + Kd_.data.cwiseProduct(e_dot_.data); 

      	for (int i = 0; i < n_joints_; i++){
           joints_[i].setCommand(tau_d_(i)); //tau_d(i) sets each individual joint
            // joints_[i].setCommand(0.0);
        }
        // if det. of a matrx gets close to 0 then use that psuedo inverse thing
        // ********* 3. data 저장 *********
       // save_data();

        // ********* 4. state (this gets printed to terminal console *********
        //print_state();
